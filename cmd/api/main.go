...
type config struct {
	...
	limiter struct {
		rps     float64 // Requests per second (ie. refill rate)
...
func main() {
	...
	srv := &http.Server{
		...
		Handler:      app.routes(),
		ErrorLog:     log.New(logger, "", 0), // Have server errors write to our logger as well.
...
func openDB(cfg config) (*sql.DB, error) {
	...
	// We will wait for 5 seconds for a response from the DB.
